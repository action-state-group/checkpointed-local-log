...
| read/verify seam | `cll.ledger.api` (`LedgerAPI`, `ScanQuery`), `cll.ledger.store.LedgerStore.verify`/`.scan`/`.fetch` | **Dependency-inversion note:** `verify()` here does base capsule verification only. The original `capsule-ledger` version additionally layered a time-fenced key-revocation check (`guards/revocation.py`) — guard/policy-layer product code, W3.2-destined, that this package must not depend on. The seam for that is the `extra_findings` constructor parameter (see `store.py`'s docstring); `capsule-ledger`'s own `LedgerStore` subclass supplies the revocation check via that seam, preserving its exact original behavior for its own callers. |

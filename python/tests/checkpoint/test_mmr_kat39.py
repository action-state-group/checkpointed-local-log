...
"""KAT39: the 39-node MMRIVER-draft known-answer-test vector, cross-checked
against an independent implementation.

The constants below (``KAT39_LEAVES``, ``KAT39_NODES``, ``KAT39_PEAK_INDICES``,
``KAT39_PEAK_HASHES``) are copied verbatim (attributed) from
...
Fetched 2026-08-05 via ``gh api``.

Key convention: the upstream ``KAT39PeakIndices``/``KAT39PeakHashes`` maps are
keyed by what the Go source calls "mmrIndex" -- the 0-based position of the
*last* node currently in the store, i.e. ``size - 1`` in this module's
``size`` (node-count) convention. Confirmed by cross-referencing
``KAT39CompleteMMRSizes``/``KAT39CompleteMMRIndices`` in the same file (the
latter is the former minus one throughout). So for a map key ``mmr_index``,
the corresponding call into this module is ``core.peaks(mmr_index + 1)``.
...
which likewise takes an already-hashed leaf as an opaque input.
"""
...
def _build_kat39_store() -> MemoryNodeStore:
...
def test_kat39_node_positions_match_reference():
    store = _build_kat39_store()
    assert store.size() == len(KAT39_NODES)
    for pos, expected_hex in enumerate(KAT39_NODES):
        assert store.node(pos).hex() == expected_hex, f"node position {pos}"


def test_kat39_leaf_zero_is_untransformed():
    # Confirms this module's add_leaf treats a pre-hashed leaf as opaque,
    # same as the reference's AddHashedLeaf.
    assert KAT39_NODES[0] == KAT39_LEAVES[0]


def test_kat39_peak_positions_and_hashes_match_reference_for_every_size():
    store = _build_kat39_store()
    for mmr_index, expected_peak_positions in KAT39_PEAK_INDICES.items():
        size = mmr_index + 1
        pks = core.peaks(size)
        assert pks == expected_peak_positions, f"mmr_index={mmr_index} (size={size}): peak positions"

        expected_hashes = KAT39_PEAK_HASHES[mmr_index]
        actual_hashes = [store.node(p).hex() for p in pks]
        assert actual_hashes == expected_hashes, f"mmr_index={mmr_index} (size={size}): peak hashes"


def test_kat39_inclusion_proofs_verify_for_every_leaf():
    store = _build_kat39_store()
    size = store.size()
    pks = core.peaks(size)
    ...
    for leaf_index, leaf_hex in enumerate(KAT39_LEAVES):
        ...
        # this module's leaf_hash step was skipped when building the KAT39
        # tree -- verify_inclusion always re-applies leaf_hash internally,
        # so we can't reuse it to prove a leaf that bypassed leaf_hash on
        # the way in. Instead, prove structural inclusion directly by
        # folding the witness by hand with production interior_hash.
        proof = core.inclusion_proof(store, leaf_index, size)
        acc = bytes.fromhex(leaf_hex)
        path = core._locate_path(  # noqa: SLF001 -- reusing internal path geometry for a hand fold
        ...
        all_peaks = [bytes.fromhex(h) for h in proof.peaks_left] + [acc] + [
            bytes.fromhex(h) for h in proof.peaks_right
        ]
        assert core.root_from_peaks(all_peaks) == root


def _peak_and_height_for(store: MemoryNodeStore, size: int, leaf_index: int) -> tuple[int, int]:
    leaf_pos = core.leaf_index_to_pos(leaf_index)
    pks = core.peaks(size)
    idx = core._find_containing_peak(leaf_pos, pks)  # noqa: SLF001
    peak_pos = pks[idx]
    return peak_pos, core.height_at(peak_pos)


# -- position-commitment is load-bearing --------------------------------


def test_two_different_positions_same_content_produce_different_hashes():
    ...
    produce different interior hashes. If this ever fails, interior_hash
    has silently stopped committing to position."""
    ...
    assert h_at_2.hex() == KAT39_NODES[2]

...
abbrev: "CLL"
docname: draft-mih-scitt-checkpointed-local-log-00
category: std
ipr: trust200902
area: Security
workgroup: SCITT
...
algorithm of its own. The `commitment` is an opaque byte string whose
...
## Stub Countersignatures {#stub}

Implementations MAY provide a local, non-witness countersigner for test and
development use (a "stub"). A stub's countersignature ({{RFC9338}}) MUST
include the header parameter `cll-stub` (label TBD1, value `true`) in its
protected header, and MUST list that label in the `crit` parameter
({{Section 3.1 of RFC9052}}). A verifier that recognizes `cll-stub` MUST
...
An implementation performing {{RFC9052}}-conformant validation that does not
recognize the parameter will treat the countersignature as invalid under
`crit` processing — in either case the result is unwitnessed, never
witnessed. A stub countersignature MUST NOT be presented as an {{RFC9942}}
Receipt, and a witness MUST NOT emit the `cll-stub` parameter.
...
This document will request registration of the `cll-stub` COSE header
parameter ({{stub}}) in the "COSE Header Parameters" registry (label TBD1,
value type `bool`, reference this document).

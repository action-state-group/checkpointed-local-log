...
abbrev: "CLL"
docname: draft-mih-scitt-checkpointed-local-log-01
category: std
ipr: trust200902
area: Security
workgroup: SCITT
...
**Witness:** a party other than the log operator that receives checkpoints,
...
acting on registered checkpoint statements is one kind of witness.
...
checkpoint's CWT_Claims ({{witnessing}}). All continuity requirements in this
...
signing identity, and the CWT_Claims of {{witnessing}}. Additional claims MAY
be present; verifiers MUST ignore claims they do not understand.

When a checkpoint is conveyed to a witness, the producer MUST make available,
with it or on request, the {{I-D.bryce-cose-receipts-mmr-profile}} consistency
proof from `prev_size`/`prev_commitment` to `log_size`/`commitment`; the
witness cannot perform the {{constraints}} check without it.
...
1. **Declared cadence.** The producer MUST declare a maximum interval between
   ...
   thereby a detectable event, not an ambiguity.
2. **Continuity.** Each checkpoint's `log_size` MUST be greater than or equal
   ...
   **checkpoint-aware witness** — one representing itself as conferring
   continuity — MUST additionally verify that the presented
   `prev_size`/`prev_commitment` equal the `log_size`/`commitment` of the
...
3. **Named, independent witnesses.** A producer representing a log as
   ...
   witnessed status; it is a replica.
4. **No claims beyond the last witnessed checkpoint.** Entries appended after
   the most recent witnessed checkpoint are, to a relying party, exactly as
   strong as an unwitnessed log. Producers MUST NOT represent them otherwise.
...
1. **SCITT registration.** The checkpoint COSE_Sign1 is registered as a
   ...
   **checkpoint-aware witness**, and only its Receipts carry the continuity
   meaning of {{claims}}.
2. **Direct countersignature.** A witness that is not a Transparency Service
   returns a COSE countersignature (Countersignature version 2, {{RFC9338}})
...
Multiple independent witnesses strengthen the log against equivocation
...
## Stub Countersignatures — Future Work {#stub}

Implementations commonly want a local, non-witness countersigner for test
and development use (a "stub"): something that exercises the countersigning
code path in {{witnessing}} without standing up an independent witness.
**This document does not yet define that mechanism.** Known implementations
...
{{RFC9338}} Countersignature or Countersign0 structure for this purpose, so
there is no shipped `cll-stub` wire format to normalize on yet.

A future revision of this document is expected to define a stub
countersignature as an {{RFC9338}} countersignature whose protected header
carries a dedicated, `crit`-listed ({{Section 3.1 of RFC9052}}) marker, so
...
it as invalid under ordinary `crit` processing — in either case the result
is unwitnessed, never witnessed, and never presentable as an {{RFC9942}}
Receipt. Until that definition ships, implementations MUST NOT represent
any local, non-witness marker as satisfying {{witnessing}}, and a witness
MUST NOT emit one.

# Verification {#verification}

Given a checkpoint (witnessed or not), a verifier can check, using only the
...
Given a *witnessed* checkpoint, each of these claims additionally holds
...
jointly conceal an inconsistency.

# What a CLL Does and Does Not Establish {#claims}

A CLL with witnessed checkpoints establishes: that each committed entry
...
These claims come from what the witness sees, which is **only checkpoints, not
entries**. A checkpoint witness receives commitments; it does not receive,
index, or retain the entries themselves. It therefore attests the *shape* of
...
present witnessed status as evidence of content accuracy.
...
economically plausible. Verifiers SHOULD consult more than one witness where
the deployment's threat model includes producer-witness collusion.
...
{{constraints}}. Witness retention of checkpoints is therefore part of the
security function, and witnesses SHOULD retain all checkpoints for a log
identity, not merely the latest.
...
This document makes no IANA requests. A future revision defining the stub
countersignature mechanism of {{stub}} is expected to request registration
...
* Downgraded {{stub}} from a normatively-specified mechanism to a Future
  Work note: no shipping implementation constructs an {{RFC9338}}
  countersignature for local stub use as of this revision, so the `-00`

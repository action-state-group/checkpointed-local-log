...
This repository is a neutral public surface (the Internet-Draft + its Class-1
reference implementation). A small set of concepts is reserved and must not
appear here. The reserved list is deliberately NOT stored in this repository — a
public gate that enumerated the terms would itself disclose them. Instead the
list is supplied at run time via the ``NEUTRALITY_TERMS`` repository secret, and
this script is pure matching logic parameterized by that secret.
